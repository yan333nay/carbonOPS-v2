...
const HEADERS = ['NOME', 'E-MAIL', 'TELEFONE', 'URL DO SITE', 'SERVIÇOS', 'DATA'];
...
async function ensureHeaders(sheets, sheetName) {
  ...
  if (firstRow[0] !== 'NOME') {
    ...
    log('Cabeçalhos criados na planilha');
  }
}
...
function saveToCSV(leads) {
  ...
  for (const l of leads) {
    const cols = [l.nome, l.email, l.telefone, l.siteUrl, l.servicos, today]
      .map(v => `"${(v || '').replace(/"/g, '""')}"`)
      .join(',');
    content += cols + '\n';
  }
